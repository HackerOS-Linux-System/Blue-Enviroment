...
use gtk::glib;
use gtk::{Align, Box, Button, CssProvider, Label, Orientation, Window};
...
fn build_ui(app: &adw::Application) {
    ...
    let clock_update = glib::clone!(#[weak] clock_label,
                                    #[upgrade_or] ControlFlow::Break,
                                    move || -> ControlFlow {
                                        clock_label.set_label(&format_time());
                                        ControlFlow::Continue
                                    });
    timeout_add_local(Duration::from_secs(1), clock_update);
    ...
    window.present();
    // Periodic updates
    let periodic_update = glib::clone!(#[weak] wifi_label, #[weak] battery_label, #[weak] audio_label,
                                       #[upgrade_or] ControlFlow::Break,
                                       move || -> ControlFlow {
                                           update_wifi_status(&wifi_label);
                                           update_battery_status(&battery_label);
    ...
    timeout_add_local(Duration::from_secs(10), periodic_update);
    // Add CSS for styling
...
fn update_wifi_status(label: &Label) {
    ...
    for device in devices {
        if let Device::WiFi(wifi) = device {
            if wifi.active_connection().is_ok() {
...
fn update_audio_status(label: &Label) {
    ...
    let volume = Rc::new(RefCell::new(0u32));
    ...
    let op = context.borrow_mut().introspect().get_sink_info_by_index(0, move |result| {
        if let ListResult::Item(info) = result {
            ...
            *vol_clone.borrow_mut() = (vol as f32 / libpulse_binding::volume::Volume::NORMAL.0 as f32 * 100.0) as u32;
        }
    });
    while op.get_state() == libpulse_binding::operation::State::Running {
        mainloop.borrow_mut().iterate(true);
    }
    label.set_label(&format!("Vol: {}%", *volume.borrow()));
}
